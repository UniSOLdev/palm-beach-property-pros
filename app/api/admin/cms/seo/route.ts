...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase.from("cms_seo").select("draft, published, updated_at, published_at").eq("id", 1).maybeSingle();
...
export async function PUT(req: Request) {
  ...
  if (!draft || typeof draft !== "object" || Array.isArray(draft)) {
    return NextResponse.json({ error: "draft object required (map of page keys to SEO fields)" }, { status: 400 });
  }
  try {
    const supabase = createServiceSupabase();
...
export async function POST() {
  ...
  try {
    const supabase = createServiceSupabase();
    const { data, error } = await supabase.from("cms_seo").select("draft").eq("id", 1).maybeSingle();
    if (error) throw error;
