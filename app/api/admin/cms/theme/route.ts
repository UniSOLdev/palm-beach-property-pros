...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase.from("cms_theme").select("draft, published, updated_at, published_at").eq("id", 1).maybeSingle();
...
export async function POST() {
  ...
  try {
    const supabase = createServiceSupabase();
    const { data, error } = await supabase.from("cms_theme").select("draft").eq("id", 1).maybeSingle();
    if (error) throw error;
