...
CREATE POLICY website_pages_admin_all ON public.website_pages FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_sections_admin_all ON public.website_sections;
CREATE POLICY website_sections_admin_all ON public.website_sections FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_media_admin_all ON public.website_media;
CREATE POLICY website_media_admin_all ON public.website_media FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_revisions_admin_all ON public.website_revisions;
CREATE POLICY website_revisions_admin_all ON public.website_revisions FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_theme_admin_all ON public.website_theme;
CREATE POLICY website_theme_admin_all ON public.website_theme FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_pages_service_all ON public.website_pages;
CREATE POLICY website_pages_service_all ON public.website_pages FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_sections_service_all ON public.website_sections;
CREATE POLICY website_sections_service_all ON public.website_sections FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_media_service_all ON public.website_media;
CREATE POLICY website_media_service_all ON public.website_media FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_revisions_service_all ON public.website_revisions;
CREATE POLICY website_revisions_service_all ON public.website_revisions FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_theme_service_all ON public.website_theme;
CREATE POLICY website_theme_service_all ON public.website_theme FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS website_pages_anon_published ON public.website_pages;
CREATE POLICY website_pages_anon_published ON public.website_pages FOR SELECT TO anon USING (status = 'published');

DROP POLICY IF EXISTS website_media_anon_read ON public.website_media;
CREATE POLICY website_media_anon_read ON public.website_media FOR SELECT TO anon USING (true);

DROP POLICY IF EXISTS website_media_storage_admin ON storage.objects;
CREATE POLICY website_media_storage_admin ON storage.objects FOR ALL TO authenticated USING (bucket_id = 'website-media') WITH CHECK (bucket_id = 'website-media');

DROP POLICY IF EXISTS website_media_storage_service ON storage.objects;
CREATE POLICY website_media_storage_service ON storage.objects FOR ALL TO service_role USING (bucket_id = 'website-media') WITH CHECK (bucket_id = 'website-media');

DROP POLICY IF EXISTS website_media_storage_anon_read ON storage.objects;
CREATE POLICY website_media_storage_anon_read ON storage.objects FOR SELECT TO anon USING (bucket_id = 'website-media');

DROP POLICY IF EXISTS admin_storage_all ON storage.objects;
CREATE POLICY admin_storage_all ON storage.objects FOR ALL TO authenticated USING (bucket_id = ANY (ARRAY['receipts','job-media','cms-media','media-library','lead-media','signed-documents','website-media'])) WITH CHECK (bucket_id = ANY (ARRAY['receipts','job-media','cms-media','media-library','lead-media','signed-documents','website-media']));

INSERT INTO public.website_pages (slug, title, page_type, seo_title, meta_description, status)
SELECT 'home', 'Homepage', 'homepage', 'Palm Beach Property Pros | Premium Property Care', 'Residential, commercial, and coastal property services in Palm Beach County.', 'draft'
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.website_sections WHERE page_id = v_page_id) THEN
    ...
    (v_page_id, 'hero', 'Hero', 0, true, '{"eyebrow": "Premium Property Operations", "headline": "Property Care for Palm Beach Living", "subheadline": "Residential, commercial, and coastal property services delivered with professional crews, modern systems, and detail-focused execution.", "imageUrl": "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=2000&q=80", "chips": ["Licensed & insured", "Residential & commercial", "Airbnb & turnover specialists", "Palm Beach County based"], "primaryCta": {"label": "Get a quote", "href": "/quote"}, "secondaryCta": {"label": "Call us", "href": "tel:15616292617"}}'::jsonb),
    (v_page_id, 'services', 'Service Lines', 1, true, '{"headline": "Complete property care", "subheadline": "Exterior, interior, and operational support under one local team.", "columns": [{"title": "Exterior care", "body": "Curb presence, glass clarity, and exterior surfaces.", "links": [{"label": "Window cleaning", "href": "/services/window-cleaning"}, {"label": "Pressure washing", "href": "/services/pressure-washing"}]}, {"title": "Interior care", "body": "Residences kept inspection-ready.", "links": [{"label": "Residential cleaning", "href": "/services/residential-cleaning"}]}, {"title": "Property support", "body": "Turnovers and onsite support.", "links": [{"label": "Airbnb turnovers", "href": "/services/airbnb-services"}]}]}'::jsonb),
    (v_page_id, 'testimonials', 'Testimonials', 2, true, '{"headline": "Trusted by Palm Beach property owners", "items": [{"quote": "Professional, reliable, and detail-focused every visit.", "author": "Homeowner, Jupiter", "rating": 5}]}'::jsonb),
    (v_page_id, 'gallery', 'Gallery', 3, true, '{"headline": "Recent projects", "items": []}'::jsonb),
    (v_page_id, 'cta', 'Final CTA', 4, true, '{"headline": "One team for everything your property needs", "body": "Call or request a quote — we respond quickly with clear scope and pricing.", "primaryCta": {"label": "Get a quote", "href": "/quote"}, "phone": "561-629-2617"}'::jsonb);
  END IF;
END $$;
