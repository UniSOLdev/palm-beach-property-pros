...
VALUES ('signed-documents', 'signed-documents', false, 10485760, ARRAY['image/png', 'image/jpeg', 'application/pdf'])
ON CONFLICT (id) DO UPDATE SET public = false, file_size_limit = EXCLUDED.file_size_limit, allowed_mime_types = EXCLUDED.allowed_mime_types;

DROP POLICY IF EXISTS admin_signed_documents ON storage.objects;
CREATE POLICY admin_signed_documents ON storage.objects FOR ALL TO authenticated USING (bucket_id = 'signed-documents') WITH CHECK (bucket_id = 'signed-documents');

DROP POLICY IF EXISTS service_role_signed_documents ON storage.objects;
CREATE POLICY service_role_signed_documents ON storage.objects FOR ALL TO service_role USING (bucket_id = 'signed-documents') WITH CHECK (bucket_id = 'signed-documents');
...
CREATE OR REPLACE FUNCTION public.log_quote_event(p_quote_id uuid, p_type text, p_note text DEFAULT NULL, p_metadata jsonb DEFAULT '{}'::jsonb)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_id uuid;
BEGIN
  INSERT INTO public.quote_events (quote_id, type, note, metadata) VALUES (p_quote_id, p_type, p_note, COALESCE(p_metadata, '{}'::jsonb)) RETURNING id INTO v_id;
  RETURN v_id;
END; $$;
...
BEGIN
  SELECT * INTO v_quote FROM public.quotes WHERE public_id = p_public_id AND archived = false;
  IF NOT FOUND THEN RETURN false; END IF;
  IF v_quote.approval_status = 'pending' THEN
    UPDATE public.quotes SET approval_status = 'viewed', viewed_at = now() WHERE id = v_quote.id;
