...
export function computeDashboardMetrics(now = new Date()) {
  const { jobs, invoices, expenses, quotes } = adminSeed;
  ...
  const unpaidInvoices = invoices.filter((i) =>
    ["Unpaid", "Partially Paid", "Overdue"].includes(i.paymentStatus),
  ).length;

  const jobsScheduled = jobs.filter(
    (j) => j.status === "Scheduled" && inMonth(j.date),
  ).length;

  const completedJobs = jobs.filter((j) =>
    ["Completed", "Paid"].includes(j.status) && inMonth(j.date),
  ).length;
  ...
  const averageJobValue =
    paidJobs.length === 0
      ? 0
      : paidJobs.reduce((acc, j) => acc + j.revenue, 0) / paidJobs.length;
  ...
  const cardZelleCollected = invoices
    .filter(
      (i) =>
        ...
        (i.paymentMethod === "Card" || i.paymentMethod === "Zelle"),
    )
    .reduce((acc, i) => acc + invoiceSubtotal(i), 0);
...
export function bestClients() {
  return adminSeed.clients
    .map((c) => {
      const rev = adminSeed.jobs
      ...
      return { client: c, revenue: rev, jobsDone };
    })
